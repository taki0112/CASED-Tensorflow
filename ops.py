...
def conv_layer(x, channels, kernel=3, stride=1, activation='relu', padding='VALID', layer_name='_conv3d') :
    with tf.name_scope(layer_name) :
        if activation == None :
            return tf.layers.conv3d(inputs=x, filters=channels, kernel_size=kernel, strides=stride, padding=padding)
        else :
            return tf.layers.conv3d(inputs=x, filters=channels, kernel_size=kernel, strides=stride, padding=padding, activation=relu)

def deconv_layer(x, channels, kernel=4, stride=2, padding='VALID', layer_name='_deconv3d') :
    with tf.name_scope(layer_name) :
        x = tf.layers.conv3d_transpose(inputs=x, filters=channels, kernel_size=kernel, strides=stride, padding=padding)
        x = x[:, 1:-1, 1:-1, 1:-1, :]
        return x
def flatten(x) :
